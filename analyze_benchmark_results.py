...
class BenchmarkAnalyzer:
            ...
            try:
                ...
                if len(filename_parts) >= 5:
                    try:
                        ...
                        print(
                            f"Warning: Could not parse filename {json_file.name}: {e}"
                        )
                    ...
                    print(
                        f"Warning: Filename {json_file.name} doesn't match expected format"
                    )
    ...
    def extract_metrics(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        metrics["concurrency"] = data.get(
            "filename_concurrency", data.get("max_concurrency", 0)
        )
        metrics["input_len"] = data.get("filename_input_len", 0)
        metrics["output_len"] = data.get("filename_output_len", 0)
        metrics["backend_name"] = data.get("filename_backend", data.get("backend", ""))

        # Execution mode information
        metrics["execution_mode"] = data.get("filename_execution_mode", "unknown")
        metrics["is_eager"] = data.get("filename_is_eager")
        ...
        metrics["total_token_throughput"] = data.get("total_token_throughput", 0)
        ...
        if metrics["total_input_tokens"] > 0 and metrics["total_output_tokens"] > 0:
            metrics["avg_input_tokens_per_request"] = metrics[
                "total_input_tokens"
            ] / max(metrics["completed_requests"], 1)
    ...
    def generate_summary_stats(self) -> None:
        ...
        groups = {}
        for result in self.results:
            backend = result.get("filename_backend", result.get("backend", "unknown"))
        ...
        for group_key, group_data in sorted(groups.items()):
            ...
            # Calculate averages
            if results:
                total_throughput = sum(r.get("request_throughput", 0) for r in results)
                ...
                print(f"  Average request throughput: {avg_throughput:.2f} req/s")
                print(f"  Average token throughput: {avg_token_throughput:.2f} tok/s")
...
def main():
    ...
    parser.add_argument(
        "--input-dir", default=".", help="Directory containing JSON benchmark results"
    )
    parser.add_argument(
        "--output", default="benchmark_results_summary.csv", help="Output CSV file path"
    )
    parser.add_argument(
