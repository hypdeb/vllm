...
class EngineArgs:
    ...
    tokenizer_pool_extra_config: dict = get_field(TokenizerPoolConfig, "extra_config")
    ...
    def add_cli_args(parser: FlexibleArgumentParser) -> FlexibleArgumentParser:
        ...
        tokenizer_group.add_argument(
            "--tokenizer-pool-size", **tokenizer_kwargs["pool_size"]
        )
        tokenizer_group.add_argument(
            "--tokenizer-pool-type", **tokenizer_kwargs["pool_type"]
        )
        tokenizer_group.add_argument(
            "--tokenizer-pool-extra-config", **tokenizer_kwargs["extra_config"]
        )
        ...
        lora_group.add_argument(
            "--long-lora-scaling-factors", **lora_kwargs["long_lora_scaling_factors"]
        )
        ...
        prompt_adapter_group.add_argument(
            "--max-prompt-adapters", **prompt_adapter_kwargs["max_prompt_adapters"]
        )
        prompt_adapter_group.add_argument(
        ...
        device_group.add_argument(
            "--device", **device_kwargs["device"], deprecated=True
        )
        ...
        scheduler_group.add_argument(
            "--num-scheduler-steps", **scheduler_kwargs["num_scheduler_steps"]
        )
        scheduler_group.add_argument(
    ...
    def create_engine_config(
        ...
        parallel_config = ParallelConfig(
            ...
            enable_multimodal_encoder_data_parallel=self.enable_multimodal_encoder_data_parallel,
        )
    ...
    def _is_v1_supported_oracle(self, model_config: ModelConfig) -> bool:
        ...
        if self.disable_async_output_proc != EngineArgs.disable_async_output_proc:
            _raise_or_fallback(
                feature_name="--disable-async-output-proc", recommend_to_remove=True
            )
            return False

        if self.num_scheduler_steps != SchedulerConfig.num_scheduler_steps:
            _raise_or_fallback(
                feature_name="--num-scheduler-steps", recommend_to_remove=True
            )
            return False

        if self.scheduler_delay_factor != SchedulerConfig.delay_factor:
            _raise_or_fallback(
                feature_name="--scheduler-delay-factor", recommend_to_remove=True
            )
            return False

        if self.guided_decoding_backend not in get_args(GuidedDecodingBackendV1):
            _raise_or_fallback(
                feature_name=f"--guided-decoding-backend={self.guided_decoding_backend}",
                recommend_to_remove=False,
            )
            return False
        ...
        if self.enable_prompt_adapter:
            _raise_or_fallback(
                feature_name="--enable-prompt-adapter", recommend_to_remove=False
            )
            return False
        ...
        if model_config.dtype not in V1_SUPPORTED_DTYPES:
            _raise_or_fallback(
                feature_name=f"--dtype {model_config.dtype}", recommend_to_remove=False
            )
            return False
        ...
        if self.speculative_config is not None:
            ...
            if not (is_ngram_enabled or is_eagle_enabled or is_medusa_enabled):
                ...
                _raise_or_fallback(
                    feature_name="Speculative Decoding", recommend_to_remove=False
                )
                return False
        ...
        if threading.current_thread() != threading.main_thread() and _warn_or_fallback(
