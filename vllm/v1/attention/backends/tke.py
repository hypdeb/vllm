...
from py_tke import (
    create_op,
    forward_inplace as _forward_inplace_impl,
...
class TkeMetadata:
    ...
    sequence_lengths_device: torch.Tensor
    # Dimensions: [num_sequences], dtype: uint32
    sequence_lengths_host: torch.Tensor

    # Input sequence lengths denotes the number of tokens that are being processed in the current batch for each sequence.
    # Dimensions: [num_sequences], dtype: uint32
    ...
    # The number of sequences in the batch. Sometimes refered to as the 'batch size' or 'number of requests'.
    # Batch size is ambiguous as it could also refer to the number of tokens, and we choose 'sequence' instead of 'request' consistently.
    num_sequences: int

    # The number of sequences in context phase.
    num_context_sequences: int

    # The number of sequences in the decode or generation phase.
    num_generation_sequences: int

    # The number of tokens in the context and generation phases.
    num_tokens: int
...
class TkeMetadataBuilder(AttentionMetadataBuilder[TkeMetadata]):
    ...
    def __init__(
        ...
        prefix_cache_configuration.maxNumSequences = (
            2048  # TODO: figure out where to get this value from...
        )
    ...
    def build(
        ...
        input_sequence_lengths_device = (
            common_attn_metadata.query_start_loc.diff().to(dtype=torch.uint32))
        return TkeMetadata(
            op=self.op,
            attention_layer_dimensions=self.attention_layer_dimensions,
            sequence_lengths_device=common_attn_metadata.seq_lens,
            ...
            num_sequences=common_attn_metadata.num_reqs,
            num_context_sequences=self._num_prefill_requests,
            num_generation_sequences=self._num_decode_requests,
            num_tokens=common_attn_metadata.num_actual_tokens,
            fp8_output_buffer=self.fp8_output_buffer,
        )


@torch.library.custom_op("tke::forward_inplace",
                         mutates_args=("output", "workspace"))
def forward_inplace(
    op: torch.
    Tensor,  # Pytorch doesn't support any custom types in custom ops for some reason.
    qkv: torch.Tensor,
    numSequences: int,
    numContextSequences: int,
    inputSequenceLengthsHost: torch.Tensor,
    inputSequenceLengthsDevice: torch.Tensor,
    sequenceLengthsDevice: torch.Tensor,
    sequenceLengthsHost: torch.Tensor,
    kvCacheBlockOffsets: torch.Tensor,
    kvCachePoolPtr: int,
    rotaryCosSin: torch.Tensor,
    output: torch.Tensor,
    workspace: torch.Tensor,
    stream: int,
) -> None:
    """Custom op wrapper for TKE forward_inplace function.
    
    This function provides a black-box interface for the TKE forward_inplace
    function that is compatible with torch.compile.
    """
    _forward_inplace_impl(
        op=op,  # type: ignore
        qkv=qkv,
        numSequences=numSequences,
        numContextSequences=numContextSequences,
        inputSequenceLengthsHost=inputSequenceLengthsHost,
        inputSequenceLengthsDevice=inputSequenceLengthsDevice,
        sequenceLengthsDevice=sequenceLengthsDevice,
        sequenceLengthsHost=sequenceLengthsHost,
        kvCacheBlockOffsets=kvCacheBlockOffsets,
        kvCachePoolPtr=kvCachePoolPtr,
...
@forward_inplace.register_fake
def _(
    op: torch.
    Tensor,  # It's totally not a tensor, but PyTorch doesn't need to know that
    qkv: torch.Tensor,
    numSequences: int,
    numContextSequences: int,
    inputSequenceLengthsHost: torch.Tensor,
    ...
    This function is used by torch.compile to understand the operation
    without actually executing it during tracing.
    """
    # This is a fake implementation that doesn't actually do anything
    # but allows torch.compile to understand the operation's effects
    pass
...
class TkeImpl(AttentionImpl):
    ...
    def forward(
        ...
        forward_inplace(
            op=attn_metadata.op,
            qkv=query,
            numSequences=attn_metadata.num_sequences,
            ...
            input_sequence_lengths_device,
            sequenceLengthsDevice=attn_metadata.sequence_lengths_device,
            sequenceLengthsHost=attn_metadata.sequence_lengths_host,
        ...
        relevant_output_buffer = attn_metadata.fp8_output_buffer[:num_tokens]
        output.copy_(relevant_output_buffer)
        return output
