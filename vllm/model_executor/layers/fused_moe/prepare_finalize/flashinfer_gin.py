...
def _validate_rank_invariant_scalar(scale: torch.Tensor, name: str) -> torch.Tensor:
    ...
    if not bool((scale == scale[0]).all().item()):
        raise ValueError(f"flashinfer_gin requires one {name} shared by all experts.")
...
class FlashInferGinPrepareAndFinalize(mk.FusedMoEPrepareAndFinalizeModular):
    ...
    def prepare(
        ...
        if a1.ndim != 2 or a1.shape[1] != self.a2a.hidden:
            raise ValueError(f"expected activations shaped [tokens, {self.a2a.hidden}]")
        ...
        if expert_map is not None and (
            expert_map.dtype != torch.int32 or expert_map.numel() < self.num_experts
        ):
    ...
    def finalize(
        ...
        if (
            ...
            raise ValueError("flashinfer_gin requires contiguous BF16 expert output.")
