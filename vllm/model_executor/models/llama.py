...
from vllm.attention.backends.abstract import AttentionType, InputLayout
...
def _parse_rope_config(
    ...
    if isinstance(rotary_embedding, YaRNScalingRotaryEmbedding):
        ...
        rope = VanillaRotaryEmbeddingConfig(
    ...
    def forward(
        ...
        match (self.input_layout, self.backend_applies_rotary_embedding):
            ...
            case (InputLayout.CONTIGUOUS_QKV, True):
                if self.do_llama_4_scaling:
                    raise ValueError(
                        "Llama 4 scaling with backend-applied RoPEis not supported yet."
                    )
                attn_output = self.attn(qkv, None, None)
            case _:
                raise ValueError(
                    f"Invalid attention specifications:"
                    "input_layout={self.input_layout},"
                    f" backend_applies_rotary_embedding="
                    f"{self.backend_applies_rotary_embedding}"
                )
