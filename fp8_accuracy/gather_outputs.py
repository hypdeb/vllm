...
def run_for_variant(
    ...
    llm.collective_rpc(
        get_set_sequence_length_callback(start_seq_len, variant, output_dir)
    )
    ...
    with open("/scratch/michelangelo.txt") as file:
        book = file.read()
    ...
    for seq_len in range(start_seq_len, end_seq_len + 1, step_seq_len):
        llm.collective_rpc(
            get_set_sequence_length_callback(seq_len, variant, output_dir)
        )
...
if __name__ == "__main__":
    ...
    for variant in ("true_fp8",):
        # for variant in ("fake_fp8",):
        GLOBAL_STATE.variant = variant
        GLOBAL_STATE.output_dir = output_dir
        GLOBAL_STATE.chunk_idx_per_layer[:] = [0] * len(
            GLOBAL_STATE.chunk_idx_per_layer
        )
        ...
        # Wait for model to shutdown properly
        time.sleep(30)
