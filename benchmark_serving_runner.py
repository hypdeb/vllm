...
class VLLMServerManager:
    ...
    def start_server(
        ...
        ] if os.getenv("NSYS_PROFILE", "0") == "1" else [])
        cmd = nsys_cmd_prefix + [
            "vllm",
            "serve",
            self.model_path,
...
class BenchmarkRunner:
    ...
    def run_benchmark(self, config: Dict,
        ...
        print(
            ...
            + (f", cuda_graph={config['cuda_graph']}" if "cuda_graph" in
               config else ""))
    ...
    for execution_mode in execution_modes:
        ...
        for full_cuda_graph in cuda_graph_configs:
            ...
            if backend_name == "flash-attn" and not enforce_eager:
                ...
                cuda_graph_desc = ("with full CUDA graph" if full_cuda_graph
            ...
            try:
                ...
                if not server_started:
                    desc_suffix = (f" {cuda_graph_desc}"
                                   if cuda_graph_desc else "")
                    print(
                    ...
                    for i, config in enumerate(test_configs):
                        ...
                        if backend_name == "flash-attn" and not enforce_eager:
                            config["cuda_graph"] = ("full" if full_cuda_graph
                                                    else "none")
                        ...
                        if not success:
                            print(
                                "Benchmark failed, checking server status...")
                            if not server_manager.is_running():
                                print(
                                ...
                                print(
                                    "Server still running, continuing with next test"
                                )
